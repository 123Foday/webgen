...
// to return list of packages and check for stript setup
export  function listPackages(_req, res) {
  res.json({
    packages: PACKAGES,
  ...
  try {
    if (!isStripeConfigured()) {
      return res.status(503).json({
        error: "Payment aren't configured. Add STRIPE_SCRETE_KEY to backend/.env"
      });
    }
