...
function stripInjectedRuntime(html) {
  if (!html) return html;
  return html
    .replace(/<script id="__webgen_link_interceptor__"[\s\S]*?<\/script>/gi, "")
    .replace(/<style id="__webgen_fix__"[\s\S]*?<\/style>/gi, "");
}
...
const SUMMARY_OPEN = "<<<WEBGEN_SUMMARY>>>";
...
//   BREVO_SENDER_NAME    — display name (optional, defaults to "webgen")
//
...
// It will send an otp to that user's email valid for 10 minutes
export async function sendOtpEmail({ to, name, code, purpose }) {
  ...
  const subject = isSignup
    ? `Verify your WebGen account — code: ${code}`
    : `Your WebGen code: ${code}`;
  const intro = isSignup
    ? "Welcome to WebGen! Use this code to verify your email and finish signing up."
  ...
  const payload = {
    sender: {
      name: process.env.BREVO_SENDER_NAME || "webgen",
...
function renderEmailHtml({ name, code, intro }) {
  ...
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:14px;padding:32px;border:1px solid #e2e8f0">
    <h1 style="margin:0 0 12px;font-size:20px">Your webgen code</h1>
...
export function verifyOtp(email, code) {
  ...
  // otp valid for 10 minutes else expires
    ...
    line_items: [
      {
        price_data: {
          ...
          product_data: {
            name: `${pkg.name} pack — ${pkg.credits} webgen credits`,
...
async function vercelRequest(token, path, init = {}) {
  ...
  return body;
}// to request to vercel for deployment
...
export async function deployToVercel({ token, projectName, html, prompt }) {
  ...
  const readme = `# ${projectName || slug}\n\nDeployed via [WebGen](https://webgen.app).\n\nOriginal prompt:\n> ${prompt || "(none)"}\n`;
...
export async function publishToGitHub({
  ...
  try {
    ...
    repo = (
      await octokit.rest.repos.createForAuthenticatedUser({
        ...
        private: isPrivate,
        auto_init: true,
        description: `Built with webgen — ${projectName || "AI-generated website"}`,
      })
    ).data;
  }
  ...
  await commitFile({
    ...
    message: alreadyExisted
      ? "chore: update site via webgen"
      : "feat: initial site from webgen",
  });
...
async function commitFile({ octokit, owner, repo, branch, path, content, message }) {
  ...
  // to create a new commit or update the previou one
...
Generated with [webgen](https://github.com/) — turn a prompt into a website.
...
---
_Edit the prompt in webgen to regenerate, then upload again to push the changes here._
`;
}