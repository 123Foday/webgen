...
const LINK_INTERCEPTOR =
  '<script id="__webgen_link_interceptor__">' +
  "(function(){" +
  "if(window.__webgenLinkInterceptor)return;" +
  "window.__webgenLinkInterceptor=true;" +
...
function injectLinkInterceptor(html) {
  if (!html || /__webgen_link_interceptor__/.test(html)) return html;
...
/* ── webgen auto-fix: prevent overlapping + force responsiveness ── */
*, *::before, *::after { box-sizing:border-box; }
...
function fixOverlappingCSS(html) {
  ...
  if (/__webgen_fix__/.test(html)) return html;
  const block = `<style id="__webgen_fix__">${OVERLAP_FIX_CSS}</style>`;
  if (/<\/head>/i.test(html)) {
...
function stripAiPageRouterScripts(html) {
  ...
  return html.replace(/<script\b[^>]*>([\s\S]*?)<\/script>/gi, (full, body) => {
    if (/__webgen/.test(full)) return full; // never touch our own
