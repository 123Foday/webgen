...
import { builderPageStyles as b } from "../assets/dummyStyles";
import {
  ArrowUp,
  Plus,
  Sparkles,
...
export default function DashboardPage() {
  ...
  const loadProjects = useCallback(() => {
    ...
    setLoadError(null);
    getProjects()
      .then((d) => setProjectsData(d.projects))
  ...
  async function createProject(body) {
    ...
    try {
      ...
      if (err?.response?.status === 402) navigate("/pricing"); // if user ran of credits
    } finally {
      setCreating(false);
    }
  }

  // to flip a project between published or not
  ...
  // scrolls to the prompt box and focuses on the  input
  ...
  // It will auto-create a project if redirect from the hero ie homepage
  useEffect(() => {
                  ...
                  <div style={{
                    ...
                    <button
                      ...
                      style={{
                        ...
                        cursor: "pointer", fontSize: 13, color: "rgba(255,255,255,0.8)",
                        textAlign: "left",
                      }}
                      onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.06)"}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                    >
                      <span>✦</span> With Prompt (AI)
                    </button>
                    ...
                    <button
                      ...
                      style={{
                        width: "100%", display: "flex", alignItems: "center", gap: 10,
                        padding: "12px 16px", background: "transparent", border: "none",
                        cursor: creatingEditor ? "not-allowed" : "pointer",
                        fontSize: 13, color: "rgba(255,255,255,0.8)", textAlign: "left",
                        opacity: creatingEditor ? 0.5 : 1,
                      }}
                      onMouseEnter={e => { if (!creatingEditor) e.currentTarget.style.background = "rgba(255,255,255,0.06)"; }}
                      onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                    >
                      <span>⬡</span> {creatingEditor ? "Creating…" : "With Visual Editor"}
                    </button>
                  </div>
                )}
              </div>
            ...
            <div className={s.grid}>
              {filtered.map((p) => (
                <ProjectCard
                  ...
                  onOpenEditor={() => navigate(`/projects/${p.id}/editor`)}
...
// small helper components for promptbox
// that takes 5 credits for first time generation and 2 for refining it
function PromptBox({
  prompt,
  setPrompt,
  onSubmit,
...
// small helper component for Project card UI.
  ...
  return (
    <Card hover className={s.card}>
      <div className={s.thumbnailWrapper}>
        ...
        <span
          className={`${s.statusBadge} ${project.published ? s.statusLive : s.statusDraft
            }`}
        >
      ...
      <div className={s.cardBody}>
        ...
        <button onClick={onOpenEditor} className={b.visualEditorButton}>
...
// Here it builds 53 weeks by 7-day if days(to get contribution)
...
// to identify which column gets on which date the contribution is done
...
// to show activity in one squre cell UI
function Cell({ day, onEnter, onLeave }) {
  ...
  return (
    <div
      ...
      className={`${s.cellBase} ${isFuture ? s.cellFuture : LEVEL_COLORS[level]
        } ${!isFuture ? s.cellHover : ""}`}
    />
  );
}
// On hover shows the number of contribution done on day UI
...
//shows cell color according to the contribution done UI
function Legend() {
  return (
