...
export default function BuilderPage() {
  ...
  const [device, setDevice] = useState("desktop"); // tablet or mobile view
  ...
  // to update the project or to edit
  ...
  // auto generate the site using the credits first time
  useEffect(() => {
    if (!project) return;
  ...
  // return the message chat coming from the AI
  ...
  // to download the site in .html format
...
// to chat with the AI the UI for that
      ...
      <div className={s.chatInputArea}>
        ...
        <div className={s.inputContainer}>
          ...
          <button
            ...
            className={`${s.sendButtonBase} ${input.trim() && !generating && !outOfCredits
              ? s.gradientButton
...
// for the assistant to render bold, bullets points and paragraph breaks
...
function Message({ role, text, isError }) {
  ...
  return (
    ...
    >
      <div
        className={`${s.avatarBase} ${isUser ? s.avatarUser : s.avatarAssistant
          }`}
      >
        {isUser ? (
      ...
      <div
        className={`${s.messageBubbleBase} ${isUser ? s.messageBubbleUser : s.messageBubbleAssistant
          }`}
      >
...
// shows animated dots while AI is generating UI
    ...
    <header className={s.topbar}>
      ...
      <div className={s.deviceToggle}>
        {devices.map(({ value, icon: Icon, label }) => (
          <button
            ...
            title={label}
            className={`${s.deviceButtonBase} ${device === value ? s.deviceButtonActive : s.deviceButtonInactive
              }`}
          >
      ...
      <div className={s.actionsContainer}>
        ...
        <button
          ...
          className={`${s.publishButtonBase} ${project.published ? s.publishButtonPublished : s.gradientButton
            }`}
        >
          {project.published ? (
            <>
...
// shows current status of the AI
