...
const TOKEN_KEY = "webgen-vercel-token";
...
export default function VercelDeployModal({
  ...
  const [remember, setRemember] = useState(false);// to remember the same token
  ...
  // minimise the box when escape key is pressed
  useEffect(() => {
    if (!open) return;
  ...
  // send the deploy request to vercel
...
// renders success screen after successful deployment
